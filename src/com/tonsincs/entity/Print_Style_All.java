...
/**
 * @ProjectName:JQueue
 * @ClassName: Print_Style_All
 * @Description: TODO(实现打印所有票号信息)
 * @author 萧达光
 * @date 2014-5-28 下午01:42:44
 * 
 * @version V1.0
 */
public class Print_Style_All extends PrintSuper {

	public Print_Style_All(String printStr) {
		super(printStr);
	...
	public void print() {
		...
		//int baudRate = Integer.parseInt(JQ_Main.OS_CONTEXT
		//		.get("PRINTER_BAUD_RATE"));
		TonsincsSerialPortsPrint tspp = new TonsincsSerialPortsPrint("COM3",
				9600);
		String comp = "广州移动";//JQ_Main.OS_CONTEXT.get("COMPANY_TITLE");
		try {

			tspp.tonsinsPrnterSetFont((byte) pp.company.getW(),
					(byte) pp.company.getH());
			tspp.tonsinsPrnterPrintLine(comp + company);// 打印公司全称名称
			tspp.tonsinsPrnterSetFont((byte) pp.sellingArea.getW(),
					(byte) pp.sellingArea.getH());
			tspp.tonsinsPrnterPrintLine(sellingArea);// 打印服务厅名称
			tspp.tonsinsPrnterLF();
			tspp.tonsinsPrnterSetFont((byte) pp.bizname.getW(),
					(byte) pp.bizname.getH());
			tspp.tonsinsPrnterPrintLine(bizname);// 打印业务名称
			tspp.tonsinsPrnterSetFont((byte) pp.ticketNo.getW(),
					(byte) pp.ticketNo.getH());
			tspp.tonsinsPrnterPrintLine(ticketNo);// 打印排队号码
			tspp.tonsinsPrnterLF();
			tspp.tonsinsPrnterSetFont((byte) pp.counter.getW(),
					(byte) pp.counter.getH());
			tspp.tonsinsPrnterPrintLine(counter);// 打印窗口号

			tspp.tonsinsPrnterSetFont((byte) pp.waitNum.getW(),
					(byte) pp.waitNum.getH());
			...
			tspp.tonsinsPrnterLF();
			String[] printContens = splitting(content,
					Sys_Constant.PRINT_LINE_NUM);
			// 打印内容，并且智能换行
			for (int i = 0; i < printContens.length; i++) {
				tspp.tonsinsPrnterSetFont((byte) pp.content.getW(),
						(byte) pp.content.getH());
			...
			for (int i = 0; i < printContens.length; i++) {
				tspp.tonsinsPrnterSetFont((byte) pp.marketing.getW(),
						(byte) pp.marketing.getH());
				tspp.tonsinsPrnterPrintLine(printContens[i]);// 打印补充内容
			}
			tspp.tonsinsPrnterSetFont((byte) 1, (byte) 2);
			tspp.tonsinsPrnterPrintLine("________________________________________________");// 打印划线
			tspp.tonsinsPrnterLF();
			// 打印取号时间
			tspp.tonsinsPrnterSetFont((byte) pp.ticketTime.getW(),
					(byte) pp.ticketTime.getH());
			tspp.tonsinsPrnterPrintLine("取号时间：" + ticketTime);

			// 打印取票手机
			tspp.tonsinsPrnterSetFont((byte) pp.phone.getW(),
					(byte) pp.phone.getH());
	...
	public static void main(String[] args) {
		...
		PrintSuper psa = new Print_Style_All(str);
		psa.print();
